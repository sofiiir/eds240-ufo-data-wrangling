...
7. **What does `fct_reorder` do when it is applied to the `shape` variable? What would have happened if this step was not performed?**
    ...
    - `fct_reorder`orders the data frame in the specified order of those shapes that were seen the most amount of times. If this step were not performed the order would be in alphabetical order. 

8. **What is the purpose of rescaling `opacity_val`? And why rescale from 0.3 to 1?**

    - Creating the `opacity_val` with `rescale` sets the value to a number between 0.3 and 1 since this is the argument given to `to`. Without the rescale argument the opacity values would be between 0 and 1. An opacity vale of 0 would have made the value invisible in the visual. 
...
9. **Describe what this data frame contains.**
    ...
    - The `df_day_hour` assigns a value from 1 to 7 for the day of the week with Sunday being assigned 1 and Saturday being assigned 7. All the days in between are assigned 2 through 6 in order. The `wday` column includes an abbreviation of the weekday. The `hour` column has a numeric value of the hour in the day that the sighting took place. The `total_daily_obs` column is the number of sightings cooresponding with the respective day of the week and time of day. The `opacity_val` assigns an opacity value calculated by dividing that specific day's total daily observations and the maximum value of any day's total daily observation. Lastly, the column `hour_lab` is included as an hour label for the visual. 
...
11. **In your own words, what is the difference between `paste0()` and `glue()`? Why did the author use `paste0` to construct `quotes` and `glue` to construct the other text elements?**
    ...
    - `paste0()` uses text exactly as it is typed and variables cannot be inside of quoations to be called. The increased functionality of `glue()` is that you can call variables within quotations marks in curly brackets. This is arguably easier to code since quotation marks can be difficult to keep track of especially when there are multiple quotation marks in just one function. Some argue reading the syntax of `glue()` is also more intuitive when reading through code. `paste0()` is used for quotes since no variables are called and the exact text is used in the visual. `glue()` is used in constructing the other elements so it is easier to use variables in creating text for the visual. 
...
12. **Explain the values provided to the `x` aesthetic for both text geoms (`shape` & `total_sightings`).**
    ...
    - The `x` value in the aesthetics is specifying the position of the text. 200 is the position given for all of the `shape` titles. The position of the value of the total sightings is determined by the number of total sightings minus 200, which places the values towards the end of the column representing the number of sightings. 
...
13. **Consider the order of `geom_*()` layers in the the above plot (`plot_us`). Why did the author order the layers in this way?**
    ...
    - The `geom_rect()` is the base layer so that the text that follows is visible above. The `geom_text()` has to be on the top in order to be visible since the filled in color would block the view of the text and values. 
...
14. **This plot includes one-letter labels for each day of the week. How is this accomplished when week days are written using their three-letter abbreviations (e.g. `Mon`, `Tue`) in the `df_day_hour` data frame?**
    ...
    - The code `str_sub(string = wday, start = 1, end = 1)` selects for only the first letter in the weekday. This text is added with `geom_text()` to the visual providing the one-letter labels for each day of the week. 
...
16. **Why do you think the author chose to convert these text elements (and also in `plot_ufo`, below!) into ggplot objects (you may consider returning to this question after you've worked your way through all of the code)?**
    ...
    - The author likely chose to convert the text into `ggplot` objects to streamline making the final visual. The different elements all being in the same format makes them work well together and work with the same packages. 
...
17. **Why does the author render `plot.caption` using `ggtext::element_markdown()`, rather than `element_text()` (like he does for rendering `plot.title` and `text`)?**
    ...
    - `element_markdown()` is used instead of `element_text()` for the caption as it creates a markdown element and a link can be added to the visual. `element_text()` creates a static visual and does not have the functionality of adding hyperlinks. This is an amazing way to create visuals with links back to your GitHub.
...
20. **During week 2, we discuss [Choosing the right graphic form](https://eds-240-data-viz.github.io/course-materials/lecture-slides/lecture2.1-choosing-graphic-forms-slides.html#/title-slide). Refer to this lecture when answering the sub-questions, below:**
    ...
    a. **What "perceptual tasks" (from Cleveland & McGill's heirarchy) must the viewer perform to extract information from these visualizations?**
        ...
        - The first visual, starting from the top, requires viewers to analyze position along common scales with the added possibility of analyzing shading and saturation. The second visual relies most on the viewer interpreting shading and saturation but the values can be read as well. The third visual also relies heavily on shading and saturation and slightly relies on an interpretation of area if we are trying to determine which day had the most sightings.
    ...
    b. **What task(s) do you think the author wanted to enable or message(s) he wanted to convey with these visualizations (see lecture 2.1, slide 16 for examples)? Be sure to note at least one task / message for each of the three data viz.**
        ...
        - The first visual is conveying the most commonly seen shapes of UFO sightings. The second visual is conveying the state with the most sightings per 10k people in the state's population. The last visual shows most clearly that the UFO sightings are occurring most often between 1 am and 5 am and the day with the most UFO sightings is Sunday.
    ...
    c. **Name at least one caveat to the "hierarchy of perceptual tasks" that the author employed to achieve a goal(s) you noted in question b?**
        ...
        - The use of shading and saturation are really effective in this visual. They clearly show which shape is the most common, which state had the most UFO sightings per 10k people in the population, and which hours of the day have the most UFO sightings. 

21. **Describe two elements of this piece that you find visually-pleasing / easy to understand / intuitive. Why?** 

    - I find the use of opacity visually pleasing and easy to understand. It easily allows me to understand what values are the highest and which values are the lowest without having to read every single number. 
    
    -   Secondly, I appreciate that the top sightings of UFOs are broken into three different categories: shape, state, and time. This allows us to understand the highest values of UFO sighting in each category. 

22. **Describe two elements of this piece that you feel could be better presented in a different way. Why?** 

    - Though the circular visual representing UFO sighting based on time is visually pleasing it is not super intuitive to understand. Before recreating this visual myself, I did not notice that the first letter of the days of the week was included. Since the letter is black in a low opacity feature it is difficult to easily make out what it says. Additionally, there are two S's which makes it hard to determine which day is which easily.
    -  I do like the opacity change however when the opacity is extremely low some of the text is difficult to read. You have to strain your eyes slightly to make out the silhouttes of the letters and numbers. One way to change this is to set the base opacity slightly higher.
...
23. **Describe two new things that you learned by interpreting / annotating this code. These could be packages, functions, or even code organizational approaches that you hadn't previously known about or considered.**
    ...
    - Again, I was most impressed by the use of opacity in the visualization. I had never considered defining opacity based on the values of a given variable in the data. I am excited to incorporate this technique into my future work. 
    
    -  The code `geofacet::facet_geo(~state)` which arranges the geographic information maintaining some semblence of the geographic orientation is a cool visual package. It adds information to otherwise disjointed visuals.  
