...
8. **What is the purpose of rescaling `opacity_val`? And why rescale from 0.3 to 1?**
    ...
    - Creating the `opacity_val` with `rescale` sets the value to a number between 0.3 and 1 since this is the argument given to `to`. Without the rescale argument the opacity values would be between 0 and 1. An opacity vale of 0 would have make the value invisible in the visual. 
...
11. **In your own words, what is the difference between `paste0()` and `glue()`? Why did the author use `paste0` to construct `quotes` and `glue` to construct the other text elements?**
    ...
    - `paste0()` removes spaces from strings when outputting text. `glue` maintains the spacing that is included with the exception of removing any leading or trailing spaces. The increased functionality of `glue()` is that you can call variables within quotations marks in curly brackets. This is arguably easier to code since quotion marks can be difficult to keep track of especially when there are multiple quotation marks in just one function. Some argue reading the syntax of `glue()` is also more intuitive. 
...
20. **During week 2, we discuss [Choosing the right graphic form](https://eds-240-data-viz.github.io/course-materials/lecture-slides/lecture2.1-choosing-graphic-forms-slides.html#/title-slide). Refer to this lecture when answering the sub-questions, below:**
    ...
    b. **What task(s) do you think the author wanted to enable or message(s) he wanted to convey with these visualizations (see lecture 2.1, slide 16 for examples)? Be sure to note at least one task / message for each of the three data viz.**
        ...
        - The first visual is conveying the most commonly seen shapes of UFO sightings. The second visual is conveying the state with the most sightings per 10k people in the state's population. The last visual shows must clearly that the UFO sightings are occurig most often between 1 am and 5 am and the day with the most UFO sightings it Sunday.
